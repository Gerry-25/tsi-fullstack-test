...
router.post('/createPlayer',verifyToken,async(req,res)=>{
    if(!req.user)
    {
        ...
        const {firstName, lastName, position, size, startContract, endContract, salary, birthDate} = req.body;
...
router.put('/updatePlayerInfo/:playerId',verifyToken,async(req,res)=>{
    ...
    const result = await PlayerModel.findByIdAndUpdate(playerId,{$set:req.body}).exec();
